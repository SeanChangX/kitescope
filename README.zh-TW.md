...
- **CPU 路徑（預設）**：上傳 **ONNX** 模型，且 **class 0 為風箏**（例如以風箏資料集訓練 YOLOv8 後匯出為 ONNX）。  
- **Coral Edge TPU（選用）**：上傳為 Edge TPU 編譯過的 **TFLite 模型**（INT8，使用 `edgetpu_compiler` 編譯）。選擇 `.tflite` 模型且有偵測到 Coral TPU 時，推論會在 TPU 上執行。
...
- **模型**：使用為 Edge TPU 編譯的 YOLO 類 TFLite 模型，且 **class 0 為風箏**。
...
- **PyCoral**：
  - 在 vision image 或環境中安裝：
    ```bash
    pip install pycoral
    ```
  - 若未安裝 `pycoral`，即使有 TPU 也會自動回退為 CPU（ONNX），不會中斷服務。
